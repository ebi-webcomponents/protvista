...
The list was originally called `groups:`, which stopped being honest once it began holding standalone tracks alongside real groups. `rows:` is the canonical name — every entry is one vertical lane, and a group is simply an expandable lane. The old `groups:` alias was removed in v5: a leftover `groups:` is now just an unknown property and fails validation.
