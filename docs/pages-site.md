...
- `csv` (a single standalone track — one row, no group) and `json` (a live UniProt API track next to the BYO file) are bring-your-own-file. The examples reference `data: ./hotspots.*`, which the loader resolves against the *page*, not the config's directory (see the path-resolution caveat in `examples/README.md`). So `presets.ts` repoints them at `./sample-data/hotspots.*`, and the demo config (`vite.demo.config.mjs`, `playgroundSampleData` plugin) serves/emits those from the canonical `examples/csv|json/hotspots.*` — no committed copy, single source of truth. That is what makes the file-backed presets actually render here.
