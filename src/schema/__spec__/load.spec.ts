...
describe('loadConfig — object input', () => {
  it('accepts an object and returns a NormalizedConfig', async () => {
    ...
    expect(normalized.categories[0].id).toBe('DOMAINS');
    expect(normalized.categories[0].tracks[0].component).toBe(
  ...
  it('uses a caller-provided registry', async () => {
    ...
    expect(normalized.categories[0].tracks[0].kind).toBe('features');
  });
