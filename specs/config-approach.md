...
- Specifying _transient_ UI state. Zoom, highlight, selection, and group collapse/expand are viewer state, not config: they change constantly, mean nothing to a second reader, and have no authored counterpart.

  **Layout is the exception, and is deliberately in scope.** Row order, track order within a row, and per-row/per-track `hidden` are all things an author can already write, so when a user rearranges them in the viewer the edit goes into the config itself rather than into a parallel overlay. `getConfig()` exports the result, which is what makes an arranged view saveable, shareable, and re-loadable — and it is what an imported dataset needs, since an import has no authored config to fall back on. Two consequences follow: movement is limited to what the config can express (rows reorder among rows, tracks within their own row — a track cannot leave its group), and what the viewer persists to `localStorage` / `?layout=` is a compact diff against the authored config, not the config itself. See `docs/src/content/docs/customize-layout.md`. This reverses the earlier position that layout was orthogonal to the schema.
...
- A kind named with a **plain domain word** (`features`, `variants`, `peptides`, `linegraph`) accepts an author-supplied file. Each declares a *family* — the file formats it reads, in `KIND_ADAPTER_VARIANTS` — so `kind: variants` draws variants whether the records come from the UniProt API or from `./my-variants.csv`.
...
The rule is enforced by a drift test (`adapter-reference.spec.ts`): a built-in kind with no family must start with a provider prefix. A kind that later gains a bring-your-own-data family should shed the prefix; a name change is the honest signal that the capability changed.

**Record shapes.** Three shapes cover every family, chosen so that what a track *draws* determines what its file must contain:
...
`variation-json` and the rest are **removed**, not deprecated. Nothing in this
repository is published yet, so no alias layer is provided; `adapter:` names a
provider or consumer-registered *transform* only.
...
> `variation-csv`: column `descripton` is not a known field — did you mean
