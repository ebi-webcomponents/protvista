...
> **Note.** CSV, TSV, and BED ship today; the JSON file shorthand is a planned addition — see [`specs/generic-format-adapters.md`](./generic-format-adapters.md).
...
| A bring-your-own **BED** file has a malformed line (fewer than 3 tab-separated columns, or a non-numeric coordinate/score) | The `bed` adapter throws a descriptive error naming the offending line by 1-based physical line number and the BED column: e.g. `bed: line 3: non-numeric start coordinate "abc" (BED column 2).` or `bed: line 1: expected at least 3 tab-separated columns (chrom, start, end), got 2.`. Blank lines and `track` / `browser` / `#` comment lines are skipped, not errors. Handled exactly like the CSV/TSV case above: the loader's per-track `try/catch` logs a `console.warn` and renders **that one track empty** while the rest of the viewer renders normally; it does not (yet) raise the fetch-level ⚠ badge / `protvista-error` surface. |
| A `data:` string shorthand is a file path with an **unrecognised extension** (e.g. `./notes.gff`)                                   | Not a known generic format, so it falls through to the sources-key rule: config validation fails with `"Unknown source key: './notes.gff' in track <groupId>/<trackId>. Known sources: ..."`. Use a hosted URL, a supported extension (`.csv` / `.tsv` / `.bed`), or the object form with an explicit `adapter:`.                                                                                                                                                                                    |
...
- [x] Generic-format adapters `features-csv`, `features-tsv`, and `bed` are pre-registered on every fresh registry. A track that points at `./x.csv` / `./x.tsv` / `./x.bed` loads end to end — inferring the adapter from the extension, fetching the file as text, and rendering on `nightingale-track-canvas` — without consumer-side adapter registration. (`features-json` is follow-up work.)
- [x] File-extension shorthand (`./hits.csv` → `features-csv`, `./hits.tsv` → `features-tsv`, `./regions.bed` → `bed`) maps to the matching pre-registered adapter; a malformed file makes the adapter throw a descriptive error naming the offending row/line (and, where applicable, column), which the loader logs while rendering that track empty.
